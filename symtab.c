...
int insert_literal(char *str, unsigned type) {
	...
	for(idx = first_empty - 1; idx > FUN_REG; idx--) {
		if(strcmp(symbol_table[idx].name, str) == 0 && symbol_table[idx].type == type)
			return idx;
	}

	// provera opsega za konstante
	long int num = atol(str);
	if(((type == INT) && (num < INT_MIN || num > INT_MAX)) || ((type == UINT) && (num < 0 || num > UINT_MAX)))
...
void print_symtab(void) {
	static const char *symbol_kinds[] = {
		"NONE", "REG", "LIT", "FUN", "VAR", "PAR", "GVAR"}; // Ovo nisu dodali
	int i, j;
	printf("\n\nSYMBOL TABLE\n");
